...
export class WorkshopsService {
    constructor(
        ...
        @InjectRepository(WorkshopReservation) private reservationsRepo: Repository<WorkshopReservation>,
        @InjectRepository(WorkshopAttendee) private attendeesRepo: Repository<WorkshopAttendee>,
        @InjectRepository(WorkshopOrderSummary) private orderSummariesRepo: Repository<WorkshopOrderSummary>,
        @InjectRepository(WorkshopOrderAttendee) private orderAttendeesRepo: Repository<WorkshopOrderAttendee>,
    ...
    async create(dto: CreateWorkshopDto) {
        ...
        if (dto.deliveryMode === "online") {
            ...
            if (facilityIds.length === 0) {
                throw new BadRequestException("At least one facilityId is required for in-person workshops");
            }
            
            for (const fid of facilityIds) {
                const facility = await this.facilitiesRepo.findOne({ where: { id: fid } });
                if (!facility) {
    ...
    async update(id: string, dto: UpdateWorkshopDto) {
        ...
        // Group discount validation
        if (dto.groupDiscountEnabled !== undefined || dto.groupDiscounts !== undefined) {
            const groupDiscountEnabled = dto.groupDiscountEnabled ?? workshop.groupDiscountEnabled;
            const groupDiscounts = dto.groupDiscounts ?? workshop.groupDiscounts ?? [];
            const baseRate = dto.standardBaseRate ? Number(dto.standardBaseRate) : Number(workshop.standardBaseRate);
        ...
        if (dto.deliveryMode !== undefined) updatePayload.deliveryMode = dto.deliveryMode;
        if (dto.status !== undefined) updatePayload.status = dto.status;
        if (dto.title !== undefined) updatePayload.title = dto.title.trim();
        if (dto.shortBlurb !== undefined) updatePayload.shortBlurb = dto.shortBlurb?.trim() || undefined;
        if (dto.coverImageUrl !== undefined) updatePayload.coverImageUrl = dto.coverImageUrl?.trim() || undefined;
        if (dto.learningObjectives !== undefined) updatePayload.learningObjectives = dto.learningObjectives ?? undefined;
        if (dto.offersCmeCredits !== undefined) updatePayload.offersCmeCredits = dto.offersCmeCredits;
        if (facilityIds !== undefined) updatePayload.facilityIds = facilityIds;
        if (dto.webinarPlatform !== undefined) updatePayload.webinarPlatform = dto.webinarPlatform?.trim() || undefined;
        if (dto.meetingLink !== undefined) updatePayload.meetingLink = dto.meetingLink?.trim() || undefined;
        if (dto.meetingPassword !== undefined) updatePayload.meetingPassword = dto.meetingPassword?.trim() || undefined;
        if (dto.autoRecordSession !== undefined) updatePayload.autoRecordSession = dto.autoRecordSession;
        if (dto.capacity !== undefined) updatePayload.capacity = dto.capacity;
        if (dto.alertAt !== undefined) updatePayload.alertAt = dto.alertAt;
        if (dto.standardBaseRate !== undefined) updatePayload.standardBaseRate = dto.standardBaseRate;
        if (dto.groupDiscountEnabled !== undefined) updatePayload.groupDiscountEnabled = dto.groupDiscountEnabled;
    ...
    async listPublic(query: PublicListWorkshopsQueryDto) {
        ...
        const reservationCounts = workshopIds.length > 0 
            ? await this.reservationsRepo
                .createQueryBuilder("r")
                .select("r.workshopId", "workshopId")
                .addSelect("SUM(r.numberOfSeats)", "reservedSeats")
                .where("r.workshopId IN (:...workshopIds)", { workshopIds })
                .andWhere("r.status != :cancelledStatus", { cancelledStatus: "cancelled" })
                .groupBy("r.workshopId")
                .getRawMany()
            : [];

        const reservationMap = new Map(
            reservationCounts.map(r => [r.workshopId, parseInt(r.reservedSeats, 10)])
        );
        ...
        const transformedData = workshops.map(workshop => {
            ...
            const totalModules = workshop.days?.reduce((sum, day) => sum + (day.segments?.length || 0), 0) || 0;
            ...
            const offerPrice = workshop.groupDiscountEnabled && workshop.groupDiscounts?.length > 0
                ? workshop.groupDiscounts.sort((a, b) => a.minimumAttendees - b.minimumAttendees)[0].groupRatePerPerson
                : null;
    ...
    async getPublicWorkshopById(id: string) {
        ...
        let totalMinutes = 0;
        const daysWithDetails = workshop.days?.map(day => {
            let dayMinutes = 0;
            const segmentsWithDetails = day.segments?.map(segment => {
                const start = segment.startTime.split(":");
                const end = segment.endTime.split(":");
                const startMinutes = parseInt(start[0]) * 60 + parseInt(start[1]);
                const endMinutes = parseInt(end[0]) * 60 + parseInt(end[1]);
        ...
        const workshopEndDate = workshop.days?.[workshop.days.length - 1]?.date || null;

        // Calculate offer price (group discount for minimum attendees)
        const sortedDiscounts = workshop.groupDiscounts?.sort((a, b) => a.minimumAttendees - b.minimumAttendees) || [];
        const offerPrice = workshop.groupDiscountEnabled && sortedDiscounts.length > 0
        ...
        return {
            ...
            data: {
                ...
                groupDiscounts: sortedDiscounts.map(d => ({
                    ...
                    savingsPerPerson: (Number(workshop.standardBaseRate) - Number(d.groupRatePerPerson)).toFixed(2),
                })),
    ...
    async createOrderSummary(userId: string, dto: CheckoutOrderSummaryDto) {
        ...
        if (!workshop) {
            throw new NotFoundException("Workshop not found or not available for booking");
        }
        ...
        if (workshop.groupDiscountEnabled && workshop.groupDiscounts?.length > 0) {
            const applicableDiscounts = workshop.groupDiscounts
                ...
                .sort((a, b) => Number(a.groupRatePerPerson) - Number(b.groupRatePerPerson));
            ...
            if (applicableDiscounts.length > 0) {
                ...
                discountInfo = {
                    ...
                    savingsPerSeat: (standardPricePerSeat - appliedPricePerSeat).toFixed(2),
                    totalSavings: ((standardPricePerSeat - appliedPricePerSeat) * numberOfSeats).toFixed(2),
                };
            }
        }
        ...
        const saved = await this.orderSummariesRepo.save(orderSummary as WorkshopOrderSummary);
    ...
    async createReservation(userId: string, dto: CreateReservationDto) {
        ...
        if (attendees.length !== dto.attendeeIds.length) {
            throw new BadRequestException("Invalid attendee IDs or attendees do not belong to your order summary");
        }
        ...
        const saved = await this.reservationsRepo.save(reservation as WorkshopReservation);
