...
export class UsersService {
    ...
    async adminListUsers(query: any) {
        ...
        const tab =
            rawTab === "student" ? "students" :
                rawTab === "students" ? "students" :
                    rawTab === "faculty" ? "faculty" :
                        rawTab === "all" ? "all" :
                            null;
        ...
        if (query.sortOrder && tab !== "students") {
            throw new BadRequestException("sortOrder can be used only with tab=students");
        }

        if (productId && tab !== "students") {
            throw new BadRequestException("productId can be used only with tab=students");
        }
